...
int
core_shared_queue_enable(struct core_sock *so, unsigned long user_addr)
{
	if (!so->shmem.addr) {
		...
		so->opt.txq.base_addr = so->shmem.addr + sizeof(struct pfq_shared_queue) + core_mpsc_queue_mem(so);
