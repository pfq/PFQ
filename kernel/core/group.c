...
core_groups_init(void)
{
	...
	for(n = 0; n < Q_CORE_MAX_GID; n++)
	{
		...
		group->stats = alloc_percpu(core_group_stats_t);
		if (group->stats == NULL) {
			goto err;
		}

		group->counters = alloc_percpu(struct core_group_counters);
...
static inline
bool __core_group_is_empty(pfq_gid_t gid)
{
        return core_group_get_all_groups_mask(gid) == 0;
}


bool
core_group_policy_access(pfq_gid_t gid, pfq_id_t id, int policy)
{
...
bool
core_group_access(pfq_gid_t gid, pfq_id_t id)
{
	struct core_group *group;
...
__core_group_init(pfq_gid_t gid)
{
	...
	core_invalidate_percpu_eligible_mask((pfq_id_t __force)0);
...
__core_group_free(pfq_gid_t gid)
{
	...
	group = core_group_get(gid);
        ...
        msleep(Q_CORE_GRACE_PERIOD);   /* sleeping is possible here: user-context */
...
static int
__core_group_join(pfq_gid_t gid, pfq_id_t id, unsigned long class_mask, int policy)
{
        struct core_group * group;
        unsigned long bit;
        long tmp = 0;
...
static int
__core_group_leave(pfq_gid_t gid, pfq_id_t id)
{
        struct core_group * group;
        long tmp;
        size_t i;
...
unsigned long
core_group_get_all_groups_mask(pfq_gid_t gid)
{
        struct core_group * group;
        long mask = 0;
        size_t i;
...
void
core_group_set_filter(pfq_gid_t gid, struct sk_filter *filter)
{
        struct core_group * group;
...
int
core_group_set_prog(pfq_gid_t gid, struct pfq_lang_computation_tree *comp, void *ctx)
{
        struct core_group * group;
        struct pfq_lang_computation_tree *old_comp;
        void *old_ctx;
...
int
core_group_join(pfq_gid_t gid, pfq_id_t id, unsigned long class_mask, int policy)
{
        struct core_group * group;
        int ret;
	...
	group = core_group_get(gid);
        ...
        ret = __core_group_join(gid, id, class_mask, policy);
...
int
core_group_join_free(pfq_id_t id, unsigned long class_mask, int policy)
{
        int n = 0;

        mutex_lock(&global->groups_lock);
        for(; n < Q_CORE_MAX_ID; n++)
        {
...
int
core_group_leave(pfq_gid_t gid, pfq_id_t id)
{
        struct core_group * group;
        int ret;
...
void
core_group_leave_all(pfq_id_t id)
{
        int n = 0;
...
unsigned long
core_group_get_groups(pfq_id_t id)
{
        unsigned long ret = 0;
        int n = 0;
        mutex_lock(&global->groups_lock);
        ...
        {
		pfq_gid_t gid = (__force pfq_gid_t)n;
                unsigned long mask = core_group_get_all_groups_mask(gid);
...
struct core_group *
core_group_get(pfq_gid_t gid)
{
        if ((__force int)gid < 0 ||
            (__force int)gid >= Q_CORE_MAX_GID)
                return NULL;
...
bool
core_group_vlan_filters_enabled(pfq_gid_t gid)
{
        struct core_group *group;
...
bool
core_group_check_vlan_filter(pfq_gid_t gid, int vid)
{
        struct core_group *group;
...
bool
core_group_toggle_vlan_filters(pfq_gid_t gid, bool value)
{
        struct core_group *group;
...
void
core_group_set_vlan_filter(pfq_gid_t gid, bool value, int vid)
{
        struct core_group *group;
